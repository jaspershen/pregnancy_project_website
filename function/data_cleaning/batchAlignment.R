...
roughAlign <- function(peak.table,
                           combine.mz.tol = 25,
                           combine.rt.tol = 30){
if(length(peak.table) == 1) return(NA)

  batch1 <- peak.table[[1]]
  batch2 <- peak.table[[2]]

  ###generate sinplifued datasets
  simple.batch1 <- simplyData(data = batch1,
                              combine.mz.tol = combine.mz.tol,
                              combine.rt.tol = combine.rt.tol)

  simple.batch2 <- simplyData(data = batch2,
                              combine.mz.tol = combine.mz.tol,
                              combine.rt.tol = combine.rt.tol)
  ...
  match.result <-SXTMTmatch(data1 = data1,
                             data2 = data2,
...
mz <- data$mz
rt <- data$rt
int <- apply(data[,-c(1:3)], 1, function(x) mean(x, na.rm = TRUE))
...
baMZplot <- function(simple.data){
  if(is.null(simple.data)) return(NULL)
  ...
  mz.error <- (simple.data[[2]]$mz - simple.data[[1]]$mz)*10^6/simple.data[[2]]$mz
  mz1 <- simple.data[[1]]$mz
  mz.error.sd <- sd(abs(mz.error))
  mz.error.sd <- paste("m/z error standard:", round(mz.error.sd, 2),"ppm")
...
baRTplot <- function(simple.data){
  ...
  rt.error.sd <- paste("RT error standard:", round(rt.error.sd, 2),"second")
...
baINTplot <- function(simple.data){
  ...
  int1 <- log(apply(simple.data[[1]][,-c(1:3)], 1, function(x) mean(x, na.rm = TRUE))+1, 10)
  int2 <- log(apply(simple.data[[2]][,-c(1:3)], 1,function(x) mean(x, na.rm = TRUE))+1, 10)
  int.error <- int2 - int1

  int.error.sd <- sd(abs(int.error))
  int.error.sd <- paste("Log10int error standard:", round(int.error.sd, 2))
...
accurateAlign <- function(peak.table,
  ...
  if(length(peak.table) == 1) return(peak.table[[1]])
  if(is.na(simple.data)) return(peak.table[[1]])
  if(class(simple.data) != "list") return(NULL)
...
align2Batch <- function(batch1, batch2,
                              mz.error.sd,
                              rt.error.sd,
                              int.error.sd,
                              fold = 4,
  ...
  int1 <- log(apply(batch1[,-c(1:3)], 1, function(x) mean(x, na.rm = TRUE))+1, 10)
  ...
  int2 <- log(apply(batch2[,-c(1:3)], 1, function(x) mean(x, na.rm = TRUE))+1, 10)
  data2 <- data.frame(data2, int2, stringsAsFactors = FALSE)
  colnames(data2)[3] <- "int"
  rownames(data2) <- batch2$name

  ##batch1 match batch2
  match.result1 <- MRImatch(data1 = data1, data2 = data2,
...
remain.idx <- unlist(lapply(unique.index1, function(x){
  ...
  temp.score <- mz.weight*temp.mz.score + rt.weight*temp.rt.score + int.weight*temp.int.score
...
##batch2 match batch1
match.result2 <- MRImatch(data1 = data2, data2 = data1,
                          mz.tol = mz.error.sd*fold,
                          rt.tol = rt.error.sd*fold,
                          rt.error.type = "abs",
                          int.tol = int.error.sd*fold)
...
##new name, mz and RT
new.mz <- data.frame(batch1[,2], batch2[,2], stringsAsFactors = FALSE)
new.mz <- apply(new.mz, 1, mean)

new.rt <- data.frame(batch1[,3], batch2[,3], stringsAsFactors = FALSE)
new.rt <- apply(new.rt, 1, mean)

new.name <- paste("M", round(new.mz), "T", round(new.rt), sep = "")
...
rm(list = c("batch1", "batch2", "new.mz",
            "new.rt", "new.name", "match.result", "match.result1",
            "match.result2"))
return(return.data)
}
...
reName <- function(name){
  ...
  for(i in 1:length(temp.name)){
    ...
    if(length(temp.idx) > 1){
      name[temp.idx] <- paste(name[temp.idx], 1:length(temp.idx), sep = "_")
    }
  }
  name
}
...
getBatchAlignmentInfo <- function(raw.data,
                                  rough.align.data,
                                  accurate.align.data){
if(length(raw.data) == 1) return("Only one batch data.")
  mz.error <- (rough.align.data[[2]]$mz - rough.align.data[[1]]$mz)*10^6/rough.align.data[[2]]$mz
  mz.error.sd <- sd(abs(mz.error))

  rt.error <- rough.align.data[[2]]$rt - rough.align.data[[1]]$rt
  rt.error.sd <- sd(abs(rt.error))

  int1 <- log(apply(rough.align.data[[1]][,-c(1:3)], 1, function(x) mean(x, na.rm = TRUE))+1, 10)
  int2 <- log(apply(rough.align.data[[2]][,-c(1:3)], 1,function(x) mean(x, na.rm = TRUE))+1, 10)
  int.error <- int2 - int1
  int.error.sd <- sd(abs(int.error))

  parameter.info <- paste("The standard for m/z, RT and log10intensity errors are",
        round(mz.error.sd,2), ",",round(rt.error.sd, 2),",",
        'and', round(int.error.sd, 2), ",respectively. So the tolerances for m/z, RT and log10intensity are", 4*round(mz.error.sd,2), ",",4*round(rt.error.sd, 2),",",
        'and', 4*round(int.error.sd, 2), ",respectively.")

  peak.info <- paste("There are ", length(raw.data), " batches. And the peak number are ",
                     paste(unlist(lapply(raw.data, nrow)), collapse = ","),
                     " respectively. ", "After batch alignment, the peak number is ",
                     nrow(accurate.align.data),".", sep = "")
