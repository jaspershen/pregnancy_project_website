...
params:
  ...
  dc.di.params: NA
  dc.os.params: NA
  dc.rsd.comparation: NA
  dc.di.rsd.comparation: NA
  dc.os.pca.plot: NA
  dc.os.zero.plot: NA
---
...
<img src = "http://shenxt.me/image/metflow%20logo3.png" class = "img-topic" width = "200" align="middle" />
...
<h4> <p style="line-height:1.5em;">**MetCube (version 1.0.2)**: Created in 2018 by [Xiaotao Shen](http://shenxt.me/cv/) from Dr. Zheng-Jiang Zhu lab, Chinese Academy of Sciences.</p> </h4>
...
parameters <- rbind(dc.mv.params,
                    dc.zero.params,
            dc.dn.params,
            dc.di.params,
            dc.os.params)
num1 <- nrow(dc.mv.params) + nrow(dc.zero.params)
num2 <- nrow(dc.dn.params)
num3 <- nrow(dc.di.params)
num4 <- nrow(dc.os.params)
  ...
  row_spec((num1+num2+num3+1):(num1+num2+num3+num4),
    ...
    group_rows("MV and zero value processing [wikipedia](https://en.wikipedia.org/wiki/Imputation_(statistics))",
               1, num1,
               label_row_css = "background-color: #D7261E; color: white;") %>%
    group_rows("Data Normalization [wikipedia](https://en.wikipedia.org/wiki/Normalization_(statistics))",
               num1 + 1, num1 + num2, 
               label_row_css = "background-color: #0D6EA6; color: white;") %>%
        group_rows("Data Integration [wikipedia](https://en.wikipedia.org/wiki/Data_integration)",
                   num1 + num2 + 1, num1 + num2 + num3, 
               label_row_css = "background-color: #2F702F; color: white;") %>%
            group_rows("Outliers sample processing [wikipedia](https://en.wikipedia.org/wiki/Outlier)",
...
library(ggplot2)
library(plotly)
if(!is.null(params$dc.rsd.comparation)){
ggplotly(p = params$dc.rsd.comparation, width = 800)  
}
```

#### **RSD comparation before and after normalization**
...
library(ggplot2)
library(plotly)
if(!is.null(params$dc.di.rsd.comparation)){
ggplotly(p = params$dc.di.rsd.comparation, width = 800)  
}
```

#### **RSD comparation before and after integration**
